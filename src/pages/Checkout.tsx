...
export function Checkout() {
    ...
    try {
      const response = await axios.get(
        "https://api.countrystatecity.in/v1/countries",
        {
          headers: {
            "X-CSCAPI-KEY":
              "Q21EZTFjYUFVNkdiN0NxYkpIVEZIWlV6eVhBdW92RlVlM2JtUWRHYg==",
          },
        }
      );
  ...
  const fetchShipping = async (formData = form) => {
    const payload = {
      country: formData.country,
      state: formData.state,
      city: formData.city, // Fixed: now uses actual form value instead of hardcoded "Cebu"
      postcode: formData.zip, // Fixed: now uses actual form value instead of hardcoded "6001"
